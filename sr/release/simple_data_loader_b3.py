...
class SimpleDataLoaderB3:
    def __init__(self, capacity=500):
        ...
        # self.yData = 0.1 * np.power(self.x3Data, 5) + 0.2*np.power(self.x2Data, 4) + 0.5*np.power(self.x1Data, 3) + 1.0*np.power(self.x2Data, 2) + 2.0*self.x3Data
        self.yData = 0.8 * np.power(self.x3Data, 5) + 1.2 * np.power(self.x2Data, 4) + 1.2 * np.power(self.x1Data, 3) + 0.9 * np.power(self.x2Data, 2) + 1.1 * self.x3Data
        return
